(() => {
  ...
  const trendData = {
    week: {
      ...
      summary: {
        journal: { value: "18", progress: 78 },
        offline: { value: "7", progress: 42 },
        media: { value: "94%", progress: 94 },
      },
      labels: [
    ...
    today: {
      ...
      summary: {
        journal: { value: "6", progress: 58 },
        offline: { value: "2", progress: 24 },
        media: { value: "88%", progress: 88 },
      },
    ...
    month: {
      ...
      summary: {
        journal: { value: "46", progress: 86 },
        offline: { value: "12", progress: 52 },
        media: { value: "91%", progress: 91 },
      },
  ...
  if (trendWidget) {
    ...
    const summaryCards = {
      journal: trendWidget.querySelector('[data-summary-card="journal"]'),
      offline: trendWidget.querySelector('[data-summary-card="offline"]'),
      media: trendWidget.querySelector('[data-summary-card="media"]'),
    }
    ...
    const renderTrend = (rangeKey) => {
      ...
      Object.entries(dataset.summary).forEach(([key, item]) => {
        if (summaryNodes[key]) summaryNodes[key].textContent = item.value
        if (summaryCards[key]) {
          summaryCards[key].style.setProperty("--summary-progress", `${item.progress}%`)
          summaryCards[key].classList.remove("is-updating")
          summaryCards[key].getBoundingClientRect()
          summaryCards[key].classList.add("is-updating")
        }
      })
